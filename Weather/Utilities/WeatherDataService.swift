...
class WeatherDataService {
    ...
    func getWeatherData(coords: CLLocationCoordinate2D, lang: String) {
        ...
        cancellables = NetworkingManager.getData(url: url).decode(type: WeatherModel.self, decoder: JSONDecoder()).receive(on: DispatchQueue.main).sink(receiveCompletion: NetworkingManager.handleCompletion, receiveValue: { returnedModel in
